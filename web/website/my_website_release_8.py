...
# ------------
# END POINT-7: http://127.0.0.1:5000/validatelogin URL MAPPED TO route('/validatelogin')
##############
@my_website_app.route("/validatelogin", methods=["POST"])
def my_validatelogin_page():
    """
    Our Plan:
    Task-1: Get username and password
    Task-2: Validate with database
    Task-3: If login success pull log data and display
    """
    # ------------
    # Task-1: Get username and password
    ##############
    ...
    if users_count > 0:
        ...
        return flask.render_template("logreport.html", my_db_data=my_db_result)
    else:
        return "Login Failed.<br><a href='/login'>Go Back</a>"

    # We can write python code inside html using below syntax
    # -----
    # - {% python statement %} use this syntax for any python statement
    #
    # - {% if some_condn %} # use this syntax for writing any block
    #   {% endif %}
    #
    # - {{python_variable}} # use this syntax for printing python variable
    #
    # - python code present inside html file will be executed by the program
    #   present inside flask framework - called TEMPLATE ENGINE
    #
    # - TEMPLATE ENGINE: Name of the TEMPLATE ENGINE used in flask is JINJA2
    ####################################
